...
方向决定**哪一端是本机**，这是最容易配错的地方：

- 反向（`-R`）：`bind_*` 在远端服务器上监听，`dest_*` 由本机的 ssh 客户端发起连接（`127.0.0.1` = 本机自己）；
- 正向（`-L`）：`bind_*` 在本机监听，`dest_*` 由远端的 sshd 发起连接（`127.0.0.1` = 远端自己）。

因此 `-R` 的目标写 `192.168.179.3` 指的是"本机能路由到的地址"，而不是远端内网；反之 `-L` 的目标才是远端内网。
界面与 CLI 都用同一套字段语义，填错时的表现是"隧道起来了但连通的是另一个东西"，不会报错，所以在表单里直接标出
"远端监听 / 本机目标"这类端侧标签，并在下方给出链路预览。
